...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("click", function (e) {
    ...
    const { serial, item, fund, classification, date, status } = cell.dataset;
    ...
    else if (status === "For Repair") statusEl.classList.add("text-warning");
...
function openScannerModal() {
  ...
  // 2. Simulan ang Camera
  startScanning();
}

function startScanning() {
  // Itago ang results, ipakita ang placeholder/camera
  ...
  // Ito ang magbubukas ng camera
  html5QrCode.start({ facingMode: "environment" }, config, (decodedText) => {
    // Kapag may na-detect na QR code:
    handleSuccessfulScan(decodedText);
  });
}

function showScannedResult(text) {
  // Patigilin ang camera kapag may na-scan na
  html5QrCode.stop().then(() => {
    document.getElementById("scanner-placeholder").style.display = "none";
    document.getElementById("reader").style.display = "none";
    document.getElementById("scanner-result").style.display = "block";

    // Idagdag ang item sa listahan gaya ng sa screenshot
    const list = document.getElementById("scanned-items-list");
    list.innerHTML = `
            <div class="item-entry">
                <p class="item-name">Hammer drill</p> <p class="item-qr">QR: ${text}</p>
            </div>
        `;
  });
}

function handleSuccessfulScan(qrCode) {
  // 1. Itigil muna ang camera para hindi mag-scan nang paulit-ulit
  html5QrCode.stop().then(() => {
    // 2. Itago ang scanner/placeholder
    document.getElementById("reader").style.display = "none";
    document.getElementById("scanner-placeholder").style.display = "none";

    // 3. Ipakita ang Result Section (yung may Cancel at Mark as Received)
    const resultSection = document.getElementById("scanner-result");
    resultSection.style.display = "block";

    // 4. I-append ang item sa listahan
    // Sa ngayon, naka-hardcode muna ang "Hammer drill"
    // pero pwede natin itong i-fetch sa database gamit ang qrCode ID
    const listContainer = document.getElementById("scanned-items-list");

    const itemHtml = `
            <div class="scanned-item-entry" style="margin-bottom: 20px; text-align: center;">
                <h2 style="font-family: 'Georgia', serif; font-size: 1.8rem; margin-bottom: 5px;">Hammer drill</h2>
                <p style="font-family: 'Georgia', serif; font-size: 1.2rem; color: #333;">QR: ${qrCode}</p>
            </div>
        `;
